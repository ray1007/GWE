...
#define MAX_EXP 6
#define MAX_SENTENCE_LENGTH 17000
#define MAX_CODE_LENGTH 40
#define NUM_RADICALS 214 // kangxi dict
...
char train_file[MAX_STRING], output_word[MAX_STRING], output_ctx[MAX_STRING], output_char[MAX_STRING];
char non_comp[MAX_STRING], char_init[MAX_STRING], char_rad[MAX_STRING], char_glyph[MAX_STRING];
...
real *syn0, *syn1, *syn1neg, *expTable, *charv, *rad_embs, *charg;
int *char2rad;
clock_t start;
...
void InitNet() {
  ...
  if (cwe_type) {
    ...
    if (use_rad) {
      a = posix_memalign((void **)&char2rad, 128, (long long)character_size * sizeof(int));
      if (char2rad == NULL) {printf("Memory allocation failed\n"); exit(1);}
      
      a = posix_memalign((void **)&rad_embs, 128, (long long)NUM_RADICALS * dim * sizeof(real)); 
      if (rad_embs == NULL) {printf("Memory allocation failed\n"); exit(1);} 
      for (a = 0; a < (long long)NUM_RADICALS * dim; a++) {
        next_random = next_random * (unsigned long long)25214903917 + 11;   
        rad_embs[a] = (((next_random & 0xFFFF) / (real)65536) - 0.5) / dim;
      }
    }
    if (use_glyph) {
       a = posix_memalign((void **)&charg, 128, (long long)character_size * dim * sizeof(real));
       if (charg == NULL) {printf("Memory allocation failed\n"); exit(1);}
    }
  }
  ...
  if (strlen(char_glyph)) {
    file = fopen(char_glyph, "r");
    fscanf(file, "%lld%lld", &a, &b);
    if (b != dim) {
      printf("Unable to load -char-glyph\n");
    } else {
      while (a--) {
        fscanf(file, "%ls", buf);
        if (wcslen(buf) != 1) continue;
...
void *TrainModelThread(void *id) {
  long long a, b, d, cw, t1, t2, word, last_word, sentence_length = 0, sentence_position = 0, charv_id, rad_id;
  ...
  long long *rad_id_list = calloc(MAX_SENTENCE_LENGTH, sizeof(long long));
  int char_list_cnt, rad_list_cnt;
  ...
  while (1) {
    ...
    if (cbow) {  //train the cbow architecture
      ...
      for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
        ...
        if (cwe_type && vocab[last_word].character_size) {
          ...
          for (c = 0; c < vocab[last_word].character_size; c++) {
            ...
            // mge: add context word rad_embs
            if (use_rad == 1) {
              rad_id = char2rad[charv_id];
              for (d = 0; d < dim; d++)
                neu1char[d] += rad_embs[d + rad_id * dim] / vocab[word].character_size;
              rad_id_list[rad_list_cnt] = rad_id;
              rad_list_cnt++;
            }
            // gwe: add context word glyph feats
            if (use_glyph == 1) {
              for (d = 0; d < dim; d++)
                neu1char[d] += charg[d + charv_id * dim] / vocab[last_word].character_size;
            }
      ...
      if (cw) {
        ...
        // mge: add target word rad_embs
        if (use_rad == 2) {
          rad_list_cnt = 0;
          for (c = 0; c < dim; c++) neu1char[c] = 0;
          for (c = 0; c < vocab[word].character_size; c++) {
            charv_id = vocab[word].character[c];
            rad_id = char2rad[charv_id];
            for (d = 0; d < dim; d++)
              neu1char[d] += rad_embs[d + rad_id * dim] / 2 / vocab[word].character_size;
            rad_id_list[rad_list_cnt] = rad_id;
            rad_list_cnt++;
          }
          for (c = 0; c < dim; c++) neu1[c] += neu1char[c];
        ...
        // gwe: add target word glyph feat.
        if (use_glyph == 2) {
          for (c = 0; c < dim; c++) neu1char[c] = 0;
          for (c = 0; c < vocab[word].character_size; c++) {
            charv_id = vocab[word].character[c];
            for (d = 0; d < dim; d++)
              neu1char[d] += charg[d + charv_id * dim] / vocab[word].character_size;
          }
          for (c = 0; c < dim; c++) neu1[c] += neu1char[c]/2;
        }
        ...
        // mge: update rad_embs.
        for (a = 0; a < rad_list_cnt; a++) {
          rad_id = rad_id_list[a];
          for (c = 0; c < dim; c++) rad_embs[c + rad_id * dim] += neu1e[c] * char_rate;
        }
      }
      ...
      for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
        ...
        if (cwe_type && vocab[last_word].character_size) {
          ...
          for (c = 0; c < vocab[last_word].character_size; c++) {
            ...
            //gwe: add context glyph feats
            if (use_glyph == 1){
              for (d = 0; d < dim; d++)
                neu1[d] += charg[d + charv_id * dim] / vocab[last_word].character_size;
            }
          }
          for (d = 0; d < dim; d++) neu1[d] /= 2;
        }
        
        //mge: add target rad_embs
        if (use_rad == 2){
          rad_list_cnt = 0;
          for (c = 0; c < vocab[word].character_size; c++) {
            charv_id = vocab[word].character[c];
            rad_id = char2rad[charv_id];
            for (d = 0; d < dim; d++)
              neu1[d] += rad_embs[d + rad_id * dim] / 2 / vocab[word].character_size;
            rad_id_list[rad_list_cnt] = rad_id;
            rad_list_cnt++;
          }
        }
        //gwe: add target glyph feats
        if (use_glyph == 2){
          for (c = 0; c < vocab[word].character_size; c++) {
            charv_id = vocab[word].character[c];
            for (d = 0; d < dim; d++)
              neu1[d] += charg[d + charv_id * dim] / vocab[word].character_size;
          }
        }
        ...
        if (use_rad){
          for (a = 0; a < rad_list_cnt; a++) {
            rad_id = rad_id_list[a];
            for (d = 0; d < dim; d++) rad_embs[d + rad_id * dim] += neu1e[d] * char_rate;
          }
        }
      }
...
int main(int argc, char **argv) {
  ...
  //if ((i = ArgPos((char *)"-output-glyph", argc, argv)) > 0) strcpy(output_ctx, argv[i + 1]);
  ...
  if ((i = ArgPos((char *)"-use-rad", argc, argv)) > 0) cwe_type = atoi(argv[i + 1]);
  if ((i = ArgPos((char *)"-char2rad", argc, argv)) > 0) strcpy(char_rad, argv[i + 1]);
  if ((i = ArgPos((char *)"-use-glyph", argc, argv)) > 0) cwe_type = atoi(argv[i + 1]);
  if ((i = ArgPos((char *)"-char-glyph", argc, argv)) > 0) strcpy(char_glyph, argv[i + 1]);
